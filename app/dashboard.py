...
# ── Inject CSS ─────────────────────────────────────────────────────────────────
# __file__ makes this work from ANY working directory + Streamlit Cloud
...
try:
    ...
    if health.status_code == 200:
        st.markdown(
            '<div class="status-badge status-online">🟢 API Online — All models loaded</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div class="status-badge status-warn">🟡 API responded with non-200 status</div>',
            unsafe_allow_html=True,
        )
except Exception:
    st.markdown(
        '<div class="status-badge status-offline">'
        '🔴 API Offline — Start with: <code>uvicorn main_api:app --reload</code>'
        '</div>',
        unsafe_allow_html=True,
    )
...
with tab1:
    ...
    with col_form:
        ...
        for i, (k, default) in enumerate(pca_defaults.items()):
            v_vals[k] = v_cols1[i % 2].number_input(k, value=default, format="%.2f", key=f"fraud_{k}")
        ...
        for i, (k, default) in enumerate(pca_defaults2.items()):
            v_vals[k] = v_cols2[i % 2].number_input(k, value=default, format="%.2f", key=f"fraud_{k}2")
        ...
        if run_fraud:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    add_alert("FRAUD", level, {
                        "amount": amount, "decision": decision, "probability": prob
                    })

                    banner_class = "banner-fraud" if decision == "FRAUD" else "banner-legit"
                    banner_icon  = "🚨" if decision == "FRAUD" else "✅"
                    st.markdown(
                        f'<div class="decision-banner {banner_class}">{banner_icon} {decision}</div>',
                        unsafe_allow_html=True,
                    )
                    st.markdown(
                        f'<div class="severity-badge" style="background:{color}">Risk Level: {level}</div>',
                        unsafe_allow_html=True,
                    )
...
with tab2:
    ...
    with col_net_result:
        ...
        if load_scan:
            base_features.update({
                "Destination Port": 0, "Flow Duration": 5000,
        ...
        if load_benign:
            base_features.update({
                "Destination Port": 443, "Flow Duration": 50000,
        ...
        if run_threat:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    st.markdown(
                        f'<div class="threat-banner" style="border-color:{color}">{icon} {threat}</div>',
                        unsafe_allow_html=True,
                    )
                    st.markdown(
                        f'<div class="severity-badge" style="background:{color}">Severity: {severity}</div>',
                        unsafe_allow_html=True,
                    )
...
with tab3:
    ...
    fig_roc.add_trace(go.Scatter(
        x=fpr, y=1 - np.exp(-8 * fpr),
        name="XGBoost Fraud (AUC=0.9986)",
        line=dict(color="#ef4444", width=2),
    ))
    fig_roc.add_trace(go.Scatter(
        x=fpr, y=1 - np.exp(-6 * fpr),
        name="RF NIDS (AUC=0.997)",
